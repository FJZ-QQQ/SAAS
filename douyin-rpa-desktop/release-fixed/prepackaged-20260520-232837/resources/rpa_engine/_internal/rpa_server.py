...
async def bind_slot(request: Request):
    ...
    if not merchant_id:
        return JSONResponse({"error": "missing merchant_id"}, status_code=400)
    try:
        result = await slot_manager.create_bind_slot(int(merchant_id))
        return result
    except Exception as e:
...
async def get_bind_status(merchant_id: int):
    result = await slot_manager.check_bind_status(merchant_id)
    return result
...
async def start_slot(request: Request):
    ...
    if not merchant_id:
        return JSONResponse({"error": "missing merchant_id"}, status_code=400)
    try:
        await slot_manager.start_monitoring(int(merchant_id))
...
async def stop_slot(request: Request):
    ...
    if not merchant_id:
        return JSONResponse({"error": "missing merchant_id"}, status_code=400)
    await slot_manager.stop_monitoring(int(merchant_id))
...
async def delete_slot(request: Request):
    ...
    if not merchant_id:
        return JSONResponse({"error": "missing merchant_id"}, status_code=400)
    await slot_manager.delete_slot(int(merchant_id))
...
async def get_all_status():
    return await slot_manager.get_all_status()
...
async def login(request: Request):
    ...
    # 验证验证码
    if not _verify_sms_code(phone, code):
    ...
    try:
        ...
        if not conn:
            ...
            add_runtime_log("登录", f"商户 #1 ({phone}) 离线模式登录成功", "success")
            return {
                "status": "ok",
                "merchant_id": 1,
                "phone": phone,
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT id, phone, balance, status FROM merchant WHERE phone = %s", (phone,))
            row = cur.fetchone()
            
            if not row:
                # 新用户自动注册
                cur.execute(
                    "INSERT INTO merchant (phone, balance, status, created_at, updated_at) VALUES (%s, 0, 1, %s, %s) RETURNING id",
                    (phone, datetime.now(), datetime.now())
                )
                new_id = cur.fetchone()[0]
                conn.commit()
                add_runtime_log("注册", f"新商户 #{new_id} ({phone}) 注册成功", "success")
                row = (new_id, phone, 0, 1)
            else:
                if row[3] != 1:
                    conn.close()
...
async def get_leads(merchant_id: int = 0):
    ...
    try:
        ...
        with conn.cursor() as cur:
            if merchant_id > 0:
                cur.execute("""
                    SELECT id, merchant_id, douyin_nickname, contact, contact_type, created_at 
                    FROM customer_lead 
                    WHERE merchant_id = %s
            ...
            for row in rows:
                leads.append({
                    ...
                    "source": f"商户 {row[1]}",
                    "user": row[2] or "未知用户",
                    "type": "手机号" if row[4] == 1 else "微信号",
                    "value": row[3] or "",
                    "time": row[5].strftime("%Y-%m-%d %H:%M") if row[5] else "",
                    "status": "已提取"
                })
        conn.close()
        return leads
...
async def get_messages(merchant_id: int = 0):
    ...
    for mid, slot_data in slot_manager.slots.items():
        # ★ 只返回该商户的消息
        if merchant_id > 0 and mid != merchant_id:
            continue
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if row:
                ...
                result = {"merchant_id": 0, "balance": 0, "status": 0, "created_at": ""}
...
async def admin_login(request: Request):
    ...
    try:
        ...
        if conn:
            with conn.cursor() as cur:
                cur.execute("SELECT id, role FROM admin_user WHERE username = %s AND password = %s", (account, password))
                row = cur.fetchone()
                if row:
                    add_runtime_log("管理员", f"管理员 {account} 登录成功", "success")
                    return {"status": "ok", "role": row[1] or "admin", "admin_id": row[0]}
            conn.close()
    except Exception as e:
        print(f"[ADMIN] DB login check failed: {e}")
    
    # 2. Fallback: .env 中的超级管理员
    if account == _ADMIN_ACCOUNT and password == _ADMIN_PASSWORD:
...
async def admin_add_admin(request: Request):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("INSERT INTO admin_user (username, password, role) VALUES (%s, %s, %s)", (username, password, role))
            conn.commit()
        conn.close()
...
ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "guangchen2026")
...
async def admin_login(request: Request):
    ...
    if account == ADMIN_ACCOUNT and password == ADMIN_PASSWORD:
        return {"status": "ok", "role": "admin", "message": "登录成功"}
    else:
        return JSONResponse({"error": "账号或密码错误"}, status_code=401)
            ...
            if conn:
                ...
                cur.execute("""
                    SELECT id, phone, balance, status, created_at,
                           (SELECT COUNT(*) FROM leads WHERE leads.merchant_id = merchants.id) as lead_count
                    FROM merchants
                    ORDER BY id
                """)
                rows = cur.fetchall()
                for r in rows:
                    merchants.append({
                        "id": r[0],
                        "phone": r[1] or f"未知-{r[0]}",
                        "balance": float(r[2]) if r[2] else 0.0,
                        "status": r[3] if r[3] is not None else 1,
                        "created_at": str(r[4])[:19] if r[4] else "",
                        "lead_count": r[5] or 0,
                    })
                conn.close()
        except Exception as e:
            print(f"[Admin] 获取商户列表失败: {e}")
    
    # 如果数据库为空或不可用，至少返回当前活跃的 slot 商户
    if not merchants:
        for mid in slot_manager.slots:
            merchants.append({
                "id": mid,
                "phone": f"本地商户-{mid}",
                "balance": 1000.0,
...
async def admin_set_balance(request: Request):
    ...
    db_url = os.getenv("DATABASE_URL", "")
    if not db_url:
        return JSONResponse({"error": "数据库未配置"}, status_code=400)
    
    try:
        conn = _connect_db(db_url)
        if conn:
            cur = conn.cursor()
            cur.execute("UPDATE merchants SET balance = %s WHERE id = %s", (balance, merchant_id))
            conn.commit()
            conn.close()
            add_runtime_log("管理员", f"商户{merchant_id}余额已更新为 ¥{balance:.2f}", "success")
            return {"status": "ok", "message": f"余额已更新为 ¥{balance:.2f}"}
        return JSONResponse({"error": "数据库连接失败"}, status_code=500)
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def admin_toggle_merchant(request: Request):
    ...
    try:
        ...
        if conn:
            cur = conn.cursor()
            cur.execute("UPDATE merchants SET status = %s WHERE id = %s", (status, merchant_id))
            conn.commit()
            conn.close()
            action = "启用" if status == 1 else "停用"
            add_runtime_log("管理员", f"商户{merchant_id}已被{action}", "success")
            return {"status": "ok", "message": f"商户已{action}"}
        return JSONResponse({"error": "数据库连接失败"}, status_code=500)
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
