...
function startPythonServer() {
  ...
  if (isDev) {
    ...
    // 生产模式固定使用随安装包打进去的后端，避免客户电脑自带 Python 缺依赖导致启动失败。
  ...
  pythonProcess.on('close', (code) => {
    ...
    if (code !== 0 && code !== null) {
      ...
      dialog.showMessageBox(mainWindow, {
        type: 'warning',
        title: '后台引擎启动失败',
        message: '光宸智能客服后台服务未能启动，可能原因：\n\n' +
          '1. 杀毒软件拦截了程序文件，请将安装目录加入白名单\n' +
          '2. 端口 8100 被其他程序占用\n' +
          '3. 安装目录权限不足，请尝试以管理员身份运行\n\n' +
          '如问题持续，请联系客服微信：OrangeAI_Support',
        buttons: ['知道了'],
      }).catch(() => {})
    }
  })
...
async function checkForUpdates() {
  ...
  const fs = require('fs')
  const updateUrl = new URL(CLOUD_API)
  const updateClient = updateUrl.protocol === 'https:' ? require('https') : require('http')
  ...
  try {
    ...
    const versionData = await new Promise((resolve, reject) => {
      const req = updateClient.get(`${CLOUD_API}/api/version`, { timeout: 5000 }, (res) => {
        let data = ''
    ...
    if (updateState?.signature === remoteSignature && updateState?.updateFailed) {
      const failedFiles = updateState?.remainingFiles || []
      const onlyDistRetry = failedFiles.length === 0 && updateState?.remainingDist && needsDistUpdate
      if (!onlyDistRetry) {
        console.log('[Update] ⚠ 同一版本上次更新校验失败，暂停自动更新避免反复闪退')
        sendStatus('更新校验未通过，已暂停自动重启，可继续使用', 'error')
        return
      }
      console.log('[Update] 上次仅前端校验未通过，本次允许重试前端更新')
    }
    if (
    ...
    for (const fname of filesToUpdate) {
      try {
        const content = await new Promise((resolve, reject) => {
          const req = updateClient.get(`${CLOUD_API}/api/update/file/${fname}`, { timeout: 10000 }, (res) => {
    ...
    if (needsDistUpdate) {
        ...
        try {
          const zipData = await new Promise((resolve, reject) => {
            const req = updateClient.get(`${CLOUD_API}/api/update/dist`, { timeout: 30000 }, (res) => {
              if (res.statusCode !== 200) {
                res.resume()
                reject(new Error(`HTTP ${res.statusCode}`))
                return
          ...
          if (!fs.existsSync(path.join(distDir, 'index.html'))) {
            throw new Error('dist.zip 解压后缺少 index.html')
          }
    ...
    if (!hasUpdated && (remainingFiles.length > 0 || remainingDist)) {
      console.log(`[Update] ⚠ 未能写入任何更新，停止等待: files=${remainingFiles.join(',')} dist=${remainingDist}`)
      sendStatus('更新下载失败，可继续使用；下次启动会重试', 'error')
      return
    }
